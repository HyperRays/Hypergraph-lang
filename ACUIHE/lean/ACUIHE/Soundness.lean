...
/--
An equation is semantically valid when it holds in every ACUIhE algebra of
the universe containing its terms, under every interpretation.
-/
def SemanticallyEquivalent {Const : Type u} {Var : Type v} {Hom : Type w}
  ...
  ∀ {α : Type (max (max u v) w)} [ACUIhE Hom α]
    ...
    left.eval interpretConst interpretVar =
      right.eval interpretConst interpretVar
